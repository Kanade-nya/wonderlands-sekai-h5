<template>
  <div class="user-following">
    ...
    <div v-else-if="followings.length === 0" class="empty-container">
      <el-empty description="暂无关注" />
    </div>
    
    <div v-else class="following-list">
      <div v-for="user in followings" :key="user.id" class="following-item" @click="navigateToUser(user.id)">
...
const fetchFollowings = async () => {
  loading.value = true;
  try {
    const response = await axios.get(`${localUrl}/user/following/${props.userId}`, {
      params: {
    ...
    if (response.status === 200) {
      followings.value = (response.data.items || []).map(user => ({
        ...user,
        loading: false
      }));
